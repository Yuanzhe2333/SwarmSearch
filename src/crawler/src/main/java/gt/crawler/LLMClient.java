...
public class LLMClient {
  ...
  public static String analyzePageContent(String content) {
    try {
        ...
        + "  {"
        + "    \"role\": \"system\","
        + "    \"content\": \"You are a helpful assistant that extracts and returns only compact JSON (no formatting, no line breaks). Your response should ONLY contain a valid JSON object with keys in the defined order: date: if any, title: if any, authors: if any , and summary: summarize what the html content has. Do not explain anything else.\""
        + "  },"
        + "  {"
        + "    \"role\": \"user\","
        + "    \"content\": \"Please extract structured JSON from the following article:\\n" + content.replace("\"", "\\\"") + "\""
        + "  }"
        + "],"
        + "\"temperature\": 0.1"
        + "}";
